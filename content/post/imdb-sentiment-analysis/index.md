...
As we see above, the predictions are roughly $60 \%$.  Some of the prediction accuracies are perfect, while some are far off.  For example, in the first experiment, we predicted a  $65 \%$ probability of the word "the" appearing in a text entry while the actual probability was  $40 \%$.  Overall, we can safely say the true probablity probably falls close to  $60 \%$.  Up next, we'll perform the same task, however we'll check the probablities of find the word "the" given a positive and negative text entry.      

Here, we're splitting the positive and negative text entries from the preiously made training sets with their appropiate sentiments.  Then, we'll use 5-fold cross validation again to check our prediction accuracies.  
...
Let $A = \textbf{positive (or negative) text entry}$ and $B = B_j = \textbf{j}^{th} \ \textbf{text entry} = \cap_{i=1}^{k_j} B_{ji}$, where $B_{ji} = \text{i}^{th} \ \textbf{word in the j}^{th} \ \textbf{text entry}$ and $k_j = \textbf{number of words in text entry j}$.  Along with our naive bayes assumption, the above equation (in terms of a positive text entry) can be written as 
